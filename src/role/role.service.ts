...
export class RoleService {
  ...
  async update(id: string, updateRoleDto: UpdateRoleDto, user: IUser): Promise<Role> {
  if (!Types.ObjectId.isValid(id)) {
    throw new BadRequestException('ID role không hợp lệ');
  }
  ...
  if (updateRoleDto.name && updateRoleDto.name !== existingRole.name) {
    const duplicateName = await this.roleModel.findOne({
  ...
  async addPermissionsToRole(
    ...
    for (const permissionId of permissionIds) {
      if (!Types.ObjectId.isValid(permissionId)) {
        throw new BadRequestException(`Permission ID ${permissionId} không hợp lệ`);
      }

      const permission = await this.permissionModel.findById(permissionId).exec();
      if (!permission) {
        throw new NotFoundException(`Permission với ID ${permissionId} không tồn tại`);
      }
  ...
  async removePermissionsByName(
    ...
    if (permissionsToRemove.length === 0) {
      throw new NotFoundException('Không tìm thấy permission nào với tên đã cung cấp');
    }
